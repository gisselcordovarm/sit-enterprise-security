...
export function exportarXLSX(nombre, columnas, filas, opciones = {}) {
  ...
  // Titulo
  rows.push('<Row><Cell ss:StyleID="title"><Data ss:Type="String">SIT \u00b7 TecnoInnova \u2014 Empresa de Seguridad</Data></Cell></Row>')
  rows.push('<Row><Cell ss:StyleID="subtitle"><Data ss:Type="String">' + xml(nombre.replace(/\.xlsx?$/, '')) + '</Data></Cell></Row>')
  rows.push('<Row><Cell ss:StyleID="spacer"><Data ss:Type="String"> </Data></Cell></Row>')

  // Encabezados
  let hdr = '<Row>'
  columnas.forEach((c) => { hdr += '<Cell ss:StyleID="head"><Data ss:Type="String">' + xml(c) + '</Data></Cell>' })
  hdr += '</Row>'
  rows.push(hdr)
  ...
  // Pie
  rows.push('<Row><Cell ss:StyleID="spacer"><Data ss:Type="String"> </Data></Cell></Row>')
  rows.push('<Row><Cell ss:StyleID="footer"><Data ss:Type="String">Total registros: ' + filas.length + '</Data></Cell></Row>')
  rows.push('<Row><Cell ss:StyleID="footer"><Data ss:Type="String">Generado: ' + new Date().toLocaleDateString('es-VE') + ' ' + new Date().toLocaleTimeString('es-VE') + '</Data></Cell></Row>')

  const tblCols = widths.map(w => '<Column ss:Width="' + (w * 7) + '"/>').join('')

  const sheetName = xml(nombre.replace(/\.xlsx?$/, ''))

  const wb = '<?xml version="1.0"?>' +
    '<?mso-application progid="Excel.Sheet"?>' +
    '<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet">' +
    '<Styles>' +
    '<Style ss:ID="title"><Font ss:Bold="1" ss:Size="14" ss:Color="#0F172A"/><Alignment ss:Horizontal="Left"/></Style>' +
    '<Style ss:ID="subtitle"><Font ss:Size="11" ss:Color="#4e47cf"/><Alignment ss:Horizontal="Left"/></Style>' +
    '<Style ss:ID="spacer"><Font ss:Size="6"/></Style>' +
    '<Style ss:ID="head"><Font ss:Bold="1" ss:Size="10" ss:Color="#FFFFFF"/><Interior ss:Color="#4e47cf" ss:Pattern="Solid"/><Alignment ss:Horizontal="Center"/></Style>' +
    '<Style ss:ID="rowEven"><Font ss:Size="10" ss:Color="#181c21"/><Interior ss:Color="#f8f9ff" ss:Pattern="Solid"/></Style>' +
    '<Style ss:ID="rowOdd"><Font ss:Size="10" ss:Color="#181c21"/><Interior ss:Color="#FFFFFF" ss:Pattern="Solid"/></Style>' +
    '<Style ss:ID="footer"><Font ss:Size="9" ss:Color="#777586" ss:Italic="1"/></Style>' +
    '</Styles>' +
    '<Worksheet ss:Name="' + sheetName + '">' +
    '<Table>' + tblCols + rows.join('') + '</Table>' +
    '</Worksheet>' +
    '</Workbook>'
